...
// (Webhook fb-messenger-webhook lo realtime; function này kéo lịch sử cũ
//  + làm lưới dự phòng chạy định kỳ.)
//
// Secrets: FB_PAGE_TOKEN (token System User có quyền pages_messaging,
//          pages_read_engagement, pages_manage_metadata trên các page)
// Gọi:
//   { probe: true }            -> kiểm tra kết nối, xem 1 hội thoại mẫu (không ghi)
//   {}                         -> kéo (mặc định 25 hội thoại mới nhất/page, 100 tin/hội thoại)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!token) return json({ ok: false, error: "Chưa cấu hình FB_PAGE_TOKEN" });
    ...
    if (body.probe) {
      ...
      for (const pg of pages) {
        try {
          const d = await gget(`${pg.page_id}/conversations`, { platform: "messenger", fields: "id,updated_time,participants", limit: "3" }, token);
          const convs = d.data || [];
          let sampleMsg = null;
          if (convs[0]) {
            const md = await gget(`${convs[0].id}/messages`, { fields: "id,message,from,created_time", limit: "3" }, token);
      ...
      do {
        ...
        try {
          d = url
            ...
            : await gget(`${pg.page_id}/conversations`, { platform: "messenger", fields: "id,updated_time,participants", limit: "25" }, token);
        ...
        for (const conv of d.data || []) {
          ...
          try {
            ...
            const md = await gget(`${conv.id}/messages`, { fields: "id,message,from,created_time,attachments", limit: String(Math.min(msgPer, 100)) }, token);
            const rows: any[] = [];
