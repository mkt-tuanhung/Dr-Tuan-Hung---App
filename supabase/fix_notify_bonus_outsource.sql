-- ============================================================
-- Editor OUTSOURCE không nhận 500k/clip duyệt -> thông báo khi duyệt chạy
-- không được ghi "— thưởng 500.000đ" cho editor outsource.
-- (Lương đã sửa ở app; file này chỉ sửa NỘI DUNG thông báo cho khớp.)
-- Chạy trong Supabase SQL Editor. Dựa trên bản notif_deeplink.sql (link kèm ID).
-- ============================================================
...
declare cname text; mid uuid; pts int; lvl text; lnk text; epos text;
begin
  ...
  -- Duyệt chạy Ads -> source Đã dựng; thưởng 500k CHỈ cho editor chính thức (không Outsource)
  if NEW.approved_to_run is true and OLD.approved_to_run is distinct from true then
    ...
    if NEW.editor_id is not null then
      select position into epos from profiles where id = NEW.editor_id;
      insert into notifications(user_id, actor_id, type, title, body, link)
      values (NEW.editor_id, NEW.ads_id, 'clip_approved',
        'Video của bạn đã được duyệt chạy Ads',
        coalesce(NEW.title, '') || case when coalesce(epos, '') = 'Outsource' then '' else ' — thưởng 500.000đ' end,
        lnk);
    end if;
  end if;
  return NEW;
