          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              {faceReady ? (
                ...
                <button onClick={() => setShowEnroll(true)}
                  className="flex items-center gap-2 mx-auto px-6 py-3 rounded-2xl bg-white/15 text-white font-semibold text-sm border border-white/30 hover:bg-white/25 transition-all active:scale-95">
                  <ScanFace className="w-4 h-4" /> Đăng ký Face ID để check-out
                </button>
              )}
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-center gap-2 flex-wrap">
              {faceReady ? (
                ...
                <button onClick={() => setShowEnroll(true)}
                  className="mt-1 inline-flex items-center gap-2 px-5 py-2 rounded-xl bg-white/20 text-white text-xs font-semibold border border-white/30 hover:bg-white/30 transition-all active:scale-95">
                  <ScanFace className="w-3.5 h-3.5" /> Đăng ký Face ID
                </button>
              )}
            </div>
            <p className="text-emerald-200 text-[10px] mt-1">Quét lại khi về để cập nhật đúng giờ ra</p>
          </div>
        )}
      </div>
              ...
              <div className="p-3 space-y-1.5">
                {isToday && !selRec && (
                  <button onClick={() => { setSelectedDay(null); if (faceReady) setFaceMode('CHECK_IN'); else setShowEnroll(true); }}
                    ...
                    <div>
                      <div className="text-sm font-medium text-slate-700">{faceReady ? 'Check-in bằng khuôn mặt' : 'Đăng ký Face ID để chấm công'}</div>
                      <div className="text-xs text-slate-400">{faceReady ? 'Quét ~1 giây · GPS ghi nhận tự động' : 'Chấm công chỉ dùng Face ID'}</div>
                    </div>
                  </button>
                )}
                {isToday && selRec && !selRec.check_out && (
                  <button onClick={() => { setSelectedDay(null); if (faceReady) setFaceMode('CHECK_OUT'); else setShowEnroll(true); }}
                    ...
                    <div>
                      <div className="text-sm font-medium text-slate-700">{faceReady ? 'Check-out bằng khuôn mặt' : 'Đăng ký Face ID để check-out'}</div>
