...
const DepositCustomerModule = () => {
  ...
  const loadData = useCallback(() => {
    ...
    const deposit = all.filter(a => {
      const s = (a.status || '').toLowerCase();
      return s === 'deposit' || s === 'cọc';
    });
    setAppointments(deposit);
  }, []);
  ...
  return (
    <>
      <Helmet><title>Khách Cọc - Dr Tuấn Hùng</title></Helmet>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <h1 className="text-3xl font-extrabold tracking-tight flex items-center gap-2">
                <Users className="w-8 h-8 text-blue-500" /> Quản lý khách Cọc
              </h1>
              <p className="text-muted-foreground mt-1">Danh sách khách hàng đã đặt cọc</p>
            </div>
              ...
              <Table>
                ...
                <TableBody>
                  {appointments.length === 0 ? (
                    <TableRow><TableCell colSpan={5} className="text-center py-8 text-muted-foreground">Không có dữ liệu khách Cọc.</TableCell></TableRow>
                  ) : (
                    appointments.map(a => (
                      <TableRow key={a.id}>
                        ...
                        <TableCell className="text-center">
                          <Badge className="bg-blue-500 hover:bg-blue-600 text-white border-transparent">Đã Cọc</Badge>
                        </TableCell>
                      </TableRow>
                    ))
                  )}
...
export default DepositCustomerModule;
