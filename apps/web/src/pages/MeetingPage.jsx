  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 space-y-2">
        <input value={title} onChange={e => setTitle(e.target.value)} onKeyDown={e => e.key === 'Enter' && createMeeting(!!schedAt)} placeholder="Tên cuộc họp mới…" className="w-full px-3.5 py-2.5 text-[15px] rounded-xl border border-slate-200 focus:border-teal-400 focus:ring-2 focus:ring-teal-100 outline-none" />
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <button onClick={() => createMeeting(!!schedAt)} disabled={creating} className="h-11 px-5 rounded-xl bg-teal-600 text-white font-bold hover:bg-teal-700 disabled:opacity-50 inline-flex items-center justify-center gap-2 whitespace-nowrap">{creating ? <Loader2 className="w-4 h-4 animate-spin" /> : schedAt ? <CalendarClock className="w-4 h-4" /> : <Plus className="w-4 h-4" />} {schedAt ? 'Lên lịch họp' : 'Tạo & vào họp ngay'}</button>
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {meetings.map(m => (
            <div key={m.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex flex-col">
              ...
              {m.scheduled_at && <div className="mt-1.5 inline-flex items-center gap-1.5 text-xs font-semibold text-teal-700 bg-teal-50 px-2 py-1 rounded-lg w-fit"><CalendarClock className="w-3.5 h-3.5" /> Lịch họp: {new Date(m.scheduled_at).toLocaleString('vi-VN')}</div>}
              ...
              <div className="mt-auto pt-3 flex flex-wrap gap-2">
                ...
                {m.status !== 'ended' && <button onClick={() => copyLink(m)} title="Copy link phòng họp" className="h-10 px-3 rounded-xl border border-slate-200 text-slate-600 text-sm font-bold hover:bg-slate-50 inline-flex items-center gap-1.5"><Link2 className="w-4 h-4" /></button>}
