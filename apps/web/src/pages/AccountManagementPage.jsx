...
import { Trash2, UserPlus, Users, Loader2, Edit, ShieldAlert } from 'lucide-react';
...
const AccountManagementPage = () => {
  ...
  return (
    <>
      <Helmet>
        <title>Quản lý tài khoản - Thẩm mỹ Dr Tuấn Hùng</title>
      </Helmet>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 max-w-6xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-border pb-6">
              <div>
                ...
                <p className="text-muted-foreground mt-2 max-w-xl">
                  Tạo mã nhân sự, cấp quyền và quản lý thông tin đăng nhập.
                </p>
              </div>
            ...
            <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-border bg-muted/20">
                <h2 className="font-bold text-foreground">Danh sách nhân sự</h2>
              </div>
              {loading ? (
                ...
                <div className="p-16 text-center text-muted-foreground flex flex-col items-center">
                  ...
                  <p className="text-lg">Chưa có tài khoản nào được tạo.</p>
                </div>
              ) : (
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-muted/10 hover:bg-muted/10">
                        <TableHead className="font-bold text-muted-foreground uppercase text-xs tracking-wider">Mã nhân sự</TableHead>
                        ...
                        <TableHead className="font-bold text-muted-foreground uppercase text-xs tracking-wider">Trạng thái thử việc</TableHead>
                      ...
                      {staff.map((record) => (
                        <TableRow key={record.id} className="group hover:bg-muted/30 transition-colors">
                          <TableCell className="font-medium font-mono text-foreground">
                            {record.username || <span className="text-muted-foreground text-xs italic">Chưa thiết lập</span>}
                          </TableCell>
                          <TableCell className="font-bold text-foreground">
                            {record.name}
                          </TableCell>
                          <TableCell className="text-muted-foreground text-sm">
                            {record.position || 'Nhân viên'}
                          </TableCell>
                          <TableCell>
                            {record.trial_status === 'on_trial' ? (
                              <div className="flex flex-col gap-1">
                                <Badge variant="outline" className="bg-amber-500/10 text-amber-600 border-amber-500/20 font-bold uppercase text-[10px] tracking-wider rounded-md w-fit">Đang thử việc</Badge>
                                {record.trial_start_date && <span className="text-xs text-muted-foreground">{new Date(record.trial_start_date).toLocaleDateString('vi-VN')}</span>}
                              </div>
                            ) : record.trial_status === 'completed' ? (
                              <Badge variant="outline" className="bg-emerald-500/10 text-emerald-600 border-emerald-500/20 font-bold uppercase text-[10px] tracking-wider rounded-md w-fit">Hoàn thành thử việc</Badge>
                            ) : (
                          ...
                          <TableCell>
                            {record.active !== false ? (
                              <Badge variant="outline" className="bg-emerald-500/10 text-emerald-600 border-emerald-500/20 font-bold uppercase text-[10px] tracking-wider rounded-md">Hoạt động</Badge>
                            ) : (
                              <Badge variant="outline" className="bg-muted text-muted-foreground border-border font-bold uppercase text-[10px] tracking-wider rounded-md">Đã khóa</Badge>
                            )}
                          </TableCell>
