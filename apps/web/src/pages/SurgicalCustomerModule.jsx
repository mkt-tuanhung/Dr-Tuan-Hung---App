...
const SurgicalCustomerModule = () => {
  ...
  const loadData = useCallback(() => {
    ...
    const surgery = all.filter(a => {
      const s = (a.status || '').toLowerCase();
      return s === 'surgery' || s === 'phẫu thuật';
    });
    setAppointments(surgery);
  }, []);
  ...
  const handleSaveReport = async (e) => {
    ...
    const payload = {
      ...
      note: 'Nhập từ module Khách Phẫu thuật',
      createdBy: user?.id,
  ...
  return (
    <>
      <Helmet><title>Khách Phẫu thuật - Dr Tuấn Hùng</title></Helmet>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <h1 className="text-3xl font-extrabold tracking-tight flex items-center gap-2">
                <Users className="w-8 h-8 text-emerald-500" /> Quản lý khách Phẫu thuật
              </h1>
              <p className="text-muted-foreground mt-1">Danh sách khách hàng đã chốt phẫu thuật</p>
            </div>
              ...
              <Table>
                ...
                <TableBody>
                  {appointments.length === 0 ? (
                    <TableRow><TableCell colSpan={5} className="text-center py-8 text-muted-foreground">Không có dữ liệu khách Phẫu thuật.</TableCell></TableRow>
                  ) : (
                    appointments.map(a => (
                      <TableRow key={a.id}>
                        ...
                        <TableCell className="text-center">
                          <Badge className="bg-emerald-500 hover:bg-emerald-600 text-white border-transparent">Phẫu thuật</Badge>
                        </TableCell>
                      </TableRow>
                    ))
                  )}
...
export default SurgicalCustomerModule;
