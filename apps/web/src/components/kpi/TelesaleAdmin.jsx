      ...
      {subTab === 'assign' && (
        <>
          <div className="bg-white border border-emerald-100 rounded-2xl shadow-sm p-5">
            <h3 className="font-bold text-emerald-700 mb-4">{form.staff_id ? 'Cập nhật' : 'Tạo mới'} KPI Telesale</h3>
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="number" min="0" value={form.target_appointments} onChange={e => setForm(f => ({ ...f, target_appointments: e.target.value }))} placeholder="VD: 50" className="w-full px-3 py-2.5 rounded-xl border border-emerald-100 bg-emerald-50/30 text-sm focus:outline-none focus:border-emerald-400" />
              </div>
              <div>
                <label className="text-sm font-medium text-slate-700 block mb-1.5">Tỉ lệ chốt hẹn mục tiêu (%) *</label>
                <input type="number" step="0.1" value={form.target_close_rate} onChange={e => setForm(f => ({ ...f, target_close_rate: e.target.value }))} placeholder="VD: 30" className="w-full px-3 py-2.5 rounded-xl border border-emerald-100 bg-emerald-50/30 text-sm focus:outline-none focus:border-emerald-400" />
              </div>
              <div>
                <label className="text-sm font-medium text-slate-700 block mb-1.5">KPI doanh thu (VNĐ) *</label>
                <input inputMode="numeric" value={fmtInput(form.target_revenue)} onChange={e => setForm(f => ({ ...f, target_revenue: e.target.value.replace(/\D/g, '') }))} placeholder="VD: 200.000.000" className="w-full px-3 py-2.5 rounded-xl border border-emerald-100 bg-emerald-50/30 text-sm focus:outline-none focus:border-emerald-400" />
              </div>
              <div>
                <label className="text-sm font-medium text-slate-700 block mb-1.5">Ghi chú</label>
                <input value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} className="w-full px-3 py-2.5 rounded-xl border border-emerald-100 bg-emerald-50/30 text-sm focus:outline-none focus:border-emerald-400" />
              </div>
            </div>
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button onClick={handleSave} disabled={saving} className="px-5 py-2 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 text-white text-sm font-semibold shadow-md disabled:opacity-50">{saving ? 'Đang lưu...' : 'Lưu KPI Telesale'}</button>
            </div>
          </div>
        ...
        <>
          <div className="grid lg:grid-cols-2 gap-5">
            <div className="bg-white border border-slate-100 rounded-2xl shadow-sm p-5">
              <h3 className="font-bold text-emerald-700 mb-4">Doanh thu theo Telesale</h3>
            ...
            <div className="bg-white border border-slate-100 rounded-2xl shadow-sm p-5">
              <h3 className="font-bold text-emerald-700 mb-4">Lịch hẹn theo Telesale</h3>
                ...
                <tbody className="divide-y divide-slate-50">
                  {rows.map(r => (
                    <tr key={r.staff.id} className="hover:bg-slate-50/50">
                      ...
                      <td className="text-center px-3 py-2.5">{r.revProgress === null ? <span className="text-amber-500 text-xs">Chưa giao</span> : <span className={`text-xs font-semibold ${r.revProgress >= 100 ? 'text-emerald-600' : r.revProgress >= 70 ? 'text-yellow-600' : 'text-red-500'}`}>{r.revProgress}%</span>}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
